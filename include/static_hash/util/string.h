...
namespace static_hash {
  ...
  template <class T> constexpr uint32_t word32le(T s) { return word32le(s, 4); }

  // convert char* buffer (fragment) to uint32_t (big-endian)
  template <class T> constexpr uint32_t word32be(T s, int len) {
  ...
  template <class T> constexpr uint32_t word32be(T s) { return word32be(s, 4); }
