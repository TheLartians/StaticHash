...
namespace static_hash {
  ...
  template <class T, bool ALLOW_PADDING = false, class C, size_t N>
  constexpr auto convertBinaryArrayType(std::array<C, N> input) {
    ...
    if constexpr (sizeof(T) > sizeof(C)) {
      ...
      for (size_t i = 0; i < arr.size(); ++i) {
        auto j = i % ratio;
        arr[i] = T(C(input[i / ratio]) >> ((ratio - j - 1) * sizeof(T) * CHAR_BIT));
      }
      return arr;
    }
  }
